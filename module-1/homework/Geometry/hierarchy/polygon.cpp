...
{
    double scal = (a.x - b.x) * (c.x - b.x) + (a.y - b.y) * (c.y - b.y); // скал€рное произведение ab X bc
...
double Polygon::perimeter() const
{
    ...
    double P = sqrt((vertices[0].x - vertices[vertices.size() - 1].x) * (vertices[0].x - vertices[vertices.size() - 1].x) +
                    (vertices[0].y - vertices[vertices.size() - 1].y) * (vertices[0].y - vertices[vertices.size() - 1].y)); // св€зь последней вершины с первой
...
bool Polygon::isConvex() const
{
    ...
    for (unsigned int i = 0; i < size; ++i)
    {
        ...
        scal = (vertices[(i + 1) % size].x - vertices[i].x) * (vertices[(i + 2) % size].y - vertices[(i + 1) % size].y) -
               (vertices[(i + 1) % size].y - vertices[i].y) * (vertices[(i + 2) % size].x - vertices[(i + 1) % size].x); // псевдоскал€рное произведение
        if (i == 0)
            sign = scal >= 0 ? true : false;
